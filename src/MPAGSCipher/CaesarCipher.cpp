...
CaesarCipher::CaesarCipher(const std::size_t key) : key_{key % alphabetSize}
{
}
...
CaesarCipher::CaesarCipher(const std::string& key) : key_{0}
{
    ...
    if (!key.empty()) {
        ...
        key_ = std::stoul(key) % alphabetSize;
    }
}
        ...
        for (size_t i{0}; i < alphabetSize; ++i) {
            if (origChar == alphabet[i]) {
                ...
                case CipherMode::Encrypt:
                    processedChar = alphabet[(i + key_) % alphabetSize];
                    break;
                case CipherMode::Decrypt:
                    processedChar = alphabet[(i + alphabetSize - key_) % alphabetSize];
                    break;
                }
            }
        }
