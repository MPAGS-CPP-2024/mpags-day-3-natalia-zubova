...
int main(int argc, char* argv[])
{
    ...
    ProgramSettings commandLineSettings{
        helpRequested, versionRequested, inputFile, outputFile, cipherKey, encrypt
    };
    ...
    if (commandLineSettings.helpRequested) {
    ...
    if (commandLineSettings.versionRequested) {
    ...
    if (!commandLineSettings.inputFile.empty()) {
        // Open the file and check that we can read from it
        std::ifstream inputStream{commandLineSettings.inputFile};
        if (!inputStream.good()) {
            std::cerr << "[error] failed to create istream on file '"
                      << commandLineSettings.inputFile << "'" << std::endl;
            return 1;
        }
    ...
    if (!commandLineSettings.cipherKey.empty()) {
        ...
        for (const auto& elem : commandLineSettings.cipherKey) {
            if (!std::isdigit(elem)) {
                std::cerr
                    << "[error] cipher key must be an unsigned long integer for Caesar cipher,\n"
                    << "        the supplied key (" << commandLineSettings.cipherKey
                    << ") could not be successfully converted" << std::endl;
                return 1;
            }
        }
        caesarKey = std::stoul(commandLineSettings.cipherKey);
    }

    // Run the Caesar cipher (using the specified key and encrypt/decrypt flag) on the input text
    std::string outputText{runCaesarCipher(inputText, caesarKey, commandLineSettings.encrypt)};

    // Output the encrypted/decrypted text to stdout/file
    if (!commandLineSettings.outputFile.empty()) {
        // Open the file and check that we can write to it
        std::ofstream outputStream{commandLineSettings.outputFile};
        if (!outputStream.good()) {
            std::cerr << "[error] failed to create ostream on file '"
                      << commandLineSettings.outputFile << "'" << std::endl;
            return 1;
        }
